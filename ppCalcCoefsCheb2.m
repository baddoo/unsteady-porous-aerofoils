function pCoefs = ppCalcCoefsCheb2(a,psi0,nf,na,dzdx)

delta = 1/2;
gam = 1/2;

tcf = myJacobiNodes(nf,gam,gam); % Generate forward collocation points
...
jQAf = weight(tcf,gam,gam).*myJacobiQ2(nf-2,gam,gam,tcf);
...
jQAa = weight(t1fun(tca),gam,gam).*myJacobiQ2(nf-2,gam,gam,t1fun(tca));
...
F = [Ff;Fa];
rhsFun = 2*pi*[dzdx(xt1(tcf));dzdx(xt2(tca))];
pCoefs = F\rhsFun;
