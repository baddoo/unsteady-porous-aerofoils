...
figure(1)
for j = 1:na
semilogx(PHI,abs(real(alph1(j,:))),CL,cols(j,:),LW, 1.5)
hold on
semilogx(PHI,abs(imag(alph1(j,:))),'--',CL,cols(j,:), LW, 1.5)
end
hold off
...
figure(2)
for j = 1:na
semilogx(RHO,abs(real(alph2(j,:))),CL,cols(j,:),LW, 1.5)
hold on
semilogx(RHO,abs(imag(alph2(j,:))),'--',CL,cols(j,:), LW, 1.5)
end
hold off
ylabel('$\frac{1}{\pi} \cot^{-1}(\psi)$',INT,'Latex')
