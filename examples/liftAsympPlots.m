...
kVec = logspace(-2,2,nk-15);
kVec = sort([kVec,logspace(-.75,.25,15)]);
...
for j = 1:nk
    ...
    if m == 5; Phifun = @(x) eps + Inf*x; rhoe = @(x) 0*x; end
...
figure(1)
clf
loglog(kVec,abs(fullLift(:,end,l)),'k',LW,2)
hold on
for m = 1:na-1
loglog(kVec,abs(fullLift(:,m,l)),'Color',cols(m,:),LW,1)
end
locsU = find(kVec>1);
locsL = find(kVec<1);
...
figure(2)
clf
loglog(kVec,abs(fullCirc(:,end,l)),'k',LW,2)
hold on
for m = 1:na-1
loglog(kVec,abs(fullCirc(:,m,l)),'Color',cols(m,:),LW,1)
hold on
end
...
text(.1,2.5,'constant',INT,'Latex',FS,30,'HorizontalAlignment','center')
    ...
    loglog(kVec(locsU),.5*abs(kVec(locsU)).^.5,'k--',LW,1)
loglog(kVec(locsL),3.5+0*kVec(locsL),'k--',LW,1)
text(.1,12,{'different $\Phi$', 'same $\rho_e$'},INT,'Latex',FS,20,'HorizontalAlignment','center');
text(10,1,'$\sqrt{k}$',INT,'Latex',FS,30,'HorizontalAlignment','center')
text(.1,4,'constant',INT,'Latex',FS,30,'HorizontalAlignment','center')
end
hold off
xlabel('$k$','Interpreter','latex')
