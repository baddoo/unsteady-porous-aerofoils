...
% if strcmp(struct.type,'cheb')
% leSing = 1/2; % Leading edge singularity
% teZero = 1/2; % Trailing edge zero    
% else
if k<inf
...
LteSingNC = -1./rhoe(xCol).*INC(:,1) ...
            + weight(xCol,-.5,.5)/pi.*QNC(:,1);
LgenNC =  -1./rhoe(xCol).*IGen ...
            + weight(xCol,0.5,0.5)/pi.*QGen;
LlesingNC = -1./rhoe(xCol).*ISing(:,1) ...
            + weight(xCol,0.5,-0.5)/pi.*QSing(:,1);
else
LteSingNC =  weight(xCol,-.5,.5)/pi.*QNC(:,1);
